...
const initializer = (
  set: (fn: (state: DetailLogState) => Partial<DetailLogState>) => void,
  get: () => DetailLogState
) => ({
  currentResult: null,
  logs: new Map(),

  startResult: (resultId: string, customerName: string, action: string) => {
  ...
  endResult: (resultId: string, totalDurationMs?: number) => {
    set((state) => {
...
export const useDetailLogStore = create<DetailLogState>()(
  devtools(initializer, { name: "DetailLogStore" })
);
